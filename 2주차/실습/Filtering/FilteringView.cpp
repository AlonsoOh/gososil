...
void CFilteringView::OnProblem3()
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for (int i = -w; i <= w; i++)
				for (int j = -w; j <= w; j++) {
					if (!(0 <= r + i && r + i < m_height && 0 <= c + j && c + j < m_width))
						continue;
					Gx[r * m_width + c] += (double)Sx[i+w][j+w]*m_imageBits[(r + i) * m_width + c + j];
					Gy[r * m_width + c] += (double)Sy[i+w][j+w]*m_imageBits[(r + i) * m_width + c + j];
				}
		...
		for (c=0; c<m_width; c++) {
			...
			for(int i = -w; i <= w; i++)
				for (int j = -w; j <= w; j++) {
					if (!(0 <= r + i && r + i < m_height && 0 <= c + j && c + j < m_width))
						continue;
					mean += kernel[i + w][j + w] * imageBits[(r + i) * m_width + c + j];
					cnt++;
				}
			//mean *= (double)(w0 * w0) / cnt;
			...
			if (!c) {
				...
				for (int i = -w; i <= w; i++)
					for (int j = -w; j <= w; j++) {
						if (!(0 <= r + i && r + i < m_height && 0 <= c + j && c + j < m_width))
							continue;
						mean += kernel[i + w][j + w] * m_imageBits[(r + i) * m_width + c + j];
						cnt++;
					}
			}
			else {
				if (c + w < m_width) {
					for (int i = -w; i <= w; i++) {
						if (!(0 <= r + i && r + i < m_height)) continue;
						mean += kernel[i + w][w0-1] * m_imageBits[(r + i) * m_width + c + w];
						cnt++;
					}
				}
				if (c - w - 1 >= 0) {
					for (int i = -w; i <= w; i++) {
						if (!(0 <= r + i && r + i < m_height)) continue;
						mean -= kernel[i + w][0] * m_imageBits[(r + i) * m_width + c - w - 1];
						cnt--;
					}
				}
			}
			m_imageBitsFiltered[r * m_width + c] = mean/* * (double)(w0 * w0) / cnt*/;
		}
	}
	QueryPerformanceCounter(&m_end);
	m_result = 1000*(m_end.QuadPart-m_start.QuadPart)/m_frequency.QuadPart;	
		...
		for (c=0; c<m_width; c++) {
			...
			for (int i = -w; i <= w; i++)
				for (int j = -w; j <= w; j++) {
					if (!(0 <= r + i && r + i < m_height && 0 <= c + j && c + j < m_width))
						medarray[idx++] = 0;
					else medarray[idx++] = m_imageBits[(r + i) * m_width + c + j];
				}
			QuickMedian(medarray, 0, idx, idx / 2);
			m_imageBitsFiltered[r*m_width + c] = medarray[idx/2];
		}
	}
	QueryPerformanceCounter(&m_end);
	m_result = 1000*(m_end.QuadPart-m_start.QuadPart)/m_frequency.QuadPart;	
