...
void Mat_Vec_Multiply_GPU(float* p_VecX, float* p_MatA, float* p_VecY_G) {
    ...
    CUDA_CALL(cudaMemcpy(copy_a, p_MatA, size, cudaMemcpyHostToDevice));
    size = n * ELEM_PER_VECTOR * sizeof(float);
    CUDA_CALL(cudaMalloc(&copy_x, size));
    CUDA_CALL(cudaMemcpy(copy_x, p_VecX, size, cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMalloc(&copy_y, size));
    dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE);
    dim3 dimGrid(32 / dimBlock.x, n / 32 / dimBlock.y);
    CHECK_TIME_INIT_GPU()
        CHECK_TIME_START_GPU()

        Mat_Vec_Multiply_Kernel << < dimGrid, dimBlock >> > (copy_y, copy_x, copy_a, ELEM_PER_VECTOR);
    CHECK_TIME_END_GPU(device_time)
        CHECK_TIME_DEST_GPU()

        CUDA_CALL(cudaMemcpy(p_VecY_G, copy_y, size, cudaMemcpyDeviceToHost));
