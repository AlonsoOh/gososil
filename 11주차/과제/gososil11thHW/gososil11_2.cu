...
__global__ void Fibonacci_Kernel(int* h_Fibo_n, int* h_Fibo_v_GPU)
{
    /*Thread Idx*/
    ...
    tmp_0 = tmp_1 = 1.0f;
    for (int i = 0; i < h_Fibo_n[tid]; i++) {
        tmp_0 *= x_0; tmp_1 *= x_1;
    }
    h_Fibo_v_GPU[tid] = (int)((tmp_0 - tmp_1) / sqrt_5 + 0.5f);
}
...
cudaError_t Fibonacci_GPU(int* h_Fibo_n, int* h_Fibo_v, int* h_Fibo_v_GPU)
{
    ...
    CUDA_CALL(cudaMalloc(&copy_n, size));
    CUDA_CALL(cudaMemcpy(copy_n, h_Fibo_n, size, cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMalloc(&copy_y, size));
    ...
    Fibonacci_Kernel << < dimGrid, dimBlock >> > (copy_n, copy_y);
    ...
    CUDA_CALL(cudaMemcpy(h_Fibo_v_GPU, copy_y, size, cudaMemcpyDeviceToHost));
