...
double** make_2d_Gaussian_Kernel(int w0, double sigma1) {
	...
	for (r = 0; r < w0; r++) {
		for (c = 0; c < w0; c++) {
			double expo = (r - w0 / 2) * (r - w0 / 2) + (c - w0 / 2) * (c - w0 / 2);
			
			expo = 0 - expo; expo /= ss;
			kernel[r][c] = exp(expo) * sc;
			kernel_sum += kernel[r][c];
		}
	}
	...
	if (kernel_sum != 1.0) {
		for (r = 0; r < w0; r++) {
			for (c = 0; c < w0; c++) {
				kernel[r][c] /= kernel_sum;
				//printf("%lf ", kernel[r][c]);
			}
			//printf("\n");
		}
		kernel_sum = 1.0;
	...
	if (PRB == 1) printf("myGaussianKernel[k_size = %d, init sigma = %f] min : %lf, max : %lf\n", w0, sigma1, min, max);
...
void my_cvSmooth(IplImage* src, IplImage* dst, int w0, double sigma1)
{
	...
	for (r = 0; r < m_height; r++) {
		for (c = 0; c < m_width; c++) {
			...
			for (int i = r - w0 / 2; i <= r + w0 / 2; i++) {
				for (int j = c - w0 / 2; j <= c + w0 / 2; j++) {
					if (i < 0 || i >= m_height || j < 0 || j >= m_width) {
						idx++;
						continue;
					}

					mean += cvGetReal2D(src, i, j) * kernel[idx / w0][idx % w0];
