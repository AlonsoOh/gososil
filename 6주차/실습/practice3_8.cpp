...
void fcn3_8(int* n, double* x, double* fvec, double* fjac, int* ldfjac, int* iflag)
{
	...
	if (*iflag == 1) {
		...
		fvec[2] = exp(-1 * x[0] * x[1]) + 20.0 * x[2] + (10.0 * M_PI - 3.0) / 3.0;
	...
	// Jacobi matrix J(x)
		...
		fjac[3] = x[2] * sin(x[1] * x[2]);	fjac[4] = -162.0 * (x[1] + 0.1);	fjac[5] = -x[0] * exp(-x[0] * x[1]);
		fjac[6] = x[1] * sin(x[2] * x[2]);	fjac[7] = cos(x[2]);				fjac[8] = 20.0;
